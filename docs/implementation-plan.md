...
4. **UX 향상**
   ...
   - Claude Code 스타일 Diff UX를 추가해 Apply/Reject/Regenerate 플로우를 제공한다.
5. **테스트와 패키징**
...
- 백엔드 응답 계약에 `changes[]` 필드를 추가해 diff + 설명을 강제하고, 미응답 시 graceful fallback을 정의한다.
- Diff Preview를 띄우는 명령과 Apply/Reject/Regenerate 핸들러를 설계한다.
- 멀티 파일 diff를 순회 적용할 수 있는 QuickPick/웹뷰 UX를 설계하고 로그/Undo 연동을 기록한다.
